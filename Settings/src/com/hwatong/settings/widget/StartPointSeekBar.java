...
public class StartPointSeekBar extends View {
    ...
    private final Bitmap middleScaleImage;
    private final int defaultRangeColor;
    private final int defaultBackgroundColor;
    private Drawable middleScaleDrawable;
    ...
    private OnStartPointSeekBarChangeListener listener;
     ...
     *
     */
    public interface OnStartPointSeekBarChangeListener {
        void onOnSeekBarValueChange(StartPointSeekBar bar,
                                           int value);

        void onStopTrackingTouch(StartPointSeekBar bar,
     ...
     */
    public void setOnStartPointSeekBarChangeListener(
            OnStartPointSeekBarChangeListener listener) {
    ...
    public StartPointSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (thumbImageDrawable == null) {
            thumbImageDrawable = getResources().getDrawable(R.drawable.volume_thumb);
        }
        ...
        if (thumbImagePressedDrawable == null) {
            thumbImagePressedDrawable = getResources().getDrawable(R.drawable.volume_thumb);
        }
        
        this.middleScaleDrawable = a.getDrawable(R.styleable.StartPointSeekBar_middlescale);
        if(middleScaleDrawable == null)
        	middleScaleDrawable = getResources().getDrawable(R.drawable.setting_equal_middle);
        ...
        this.defaultRangeColor = a.getColor(R.styleable.StartPointSeekBar_defaultBackgroundRangeColor,
                DEFAULT_RANGE_COLOR);
        this.middleScaleImage = ((BitmapDrawable)middleScaleDrawable).getBitmap();
        ...
        lineHeight = 1.0f * thumbHalfHeight;
...
//                    return super.onTouchEvent(event);
            ...
            case MotionEvent.ACTION_MOVE:
                if (isThumbPressed) {
                    ...
                    if (notifyWhileDragging && listener != null) {
                        listener.onOnSeekBarValueChange(this, (int)normalizedToValue(normalizedThumbValue));
                    }
                }
                break;
            case MotionEvent.ACTION_UP:
                ...
                invalidate();
                if (listener != null) {
                	double doubleValue = normalizedToValue(normalizedThumbValue);
                	int ceilValue = 0;
                	if(Math.abs(doubleValue) > 0 && Math.abs(doubleValue) < 1) {
//                		if(doubleValue < 0)
//                    		ceilValue = (int)Math.floor(doubleValue);
//                    	else
//                    		ceilValue = (int)Math.ceil(doubleValue);
//                		LogUtils.d("doubleValue: " + doubleValue + ", ceilValue: " + ceilValue + ", " + Math.abs((doubleValue - ceilValue)));
//                		if(Math.abs((doubleValue - ceilValue)) > 0.96) {
//                			LogUtils.d("短距离数值补偿");
//                			setProgress(ceilValue); 
//                		}
                		setProgress(0);
                	}               	
//                    listener.onOnSeekBarValueChange(this, normalizedToValue(normalizedThumbValue));
                	
                    listener.onStopTrackingTouch(this, (int)normalizedToValue(normalizedThumbValue), true);
                }
                break;
...
//                    listener.onOnSeekBarValueChange(this, normalizedToValue(normalizedThumbValue));
    ...
    public void setMiddleScaleDrawableInvisible() {
    	this.middleScaleDrawable.setAlpha(0);
    }
    ...
    public void setProgress(double value) {
        ...
        invalidate();
        if (notifyWhileDragging && listener != null) {
            listener.onStopTrackingTouch(this, (int)normalizedToValue(normalizedThumbValue), false);
        }
    }

    public void setProgressValue(int value) {
        int newThumbValue = (int)valueToNormalized(value);
        if (newThumbValue > (int)absoluteMaxValue || newThumbValue < (int)absoluteMinValue) {
            throw new IllegalArgumentException("Value should be in the middle of max and min value");
        }
    ...
    public int getSeekBarMinValue() {
        return (int)absoluteMinValue;
    }

    public int getSeekBarMaxValue(){
        return (int)absoluteMaxValue;
    }
    ...
    public int getProcess() {
        return (int)normalizedToValue(normalizedThumbValue);
    }
...
//        canvas.drawRect(rect, paint);
        ...
        if (normalizedToScreen(valueToNormalized(0.0d)) < normalizedToScreen(normalizedThumbValue)) {
            ...
            int value = ((int)rect.left);
            Log.i("ljwtest", "第一次转成整形是" + value);
            float value2 = (float) value;
            Log.i("ljwtest", "第二次转成整形是" + value2);
...
//        canvas.drawRect(rect, paint);
    ...
    /**
     * Draws the middle scale bar
     * 
     */
    private void drawMiddleScale(Canvas canvas) {
    	canvas.drawBitmap(middleScaleImage, getWidth() / 2 - 1, getHeight() / 7, paint);
    }
